...
from theatre.models import Ticket, Reservation, Performance, Play, Genre, TheatreHall
...
def test_list_tickets_as_authenticated_user():
    ...
    Ticket.objects.create(performance=perf, reservation=reservation, row=1, seat=5)
...
def test_create_ticket_as_authenticated_user():
    ...
    assert Ticket.objects.filter(reservation=reservation, performance=perf).exists()
...
def test_create_ticket_as_anonymous():
    ...
    perf = Performance.objects.create(play=play, theatre_hall=hall, show_time="2026-07-30T19:00:00Z")
    reservation = Reservation.objects.create(user=User.objects.create_user("user", "u@example.com", "pass1234"))
...
def test_user_cannot_access_other_tickets():
    ...
    ticket = Ticket.objects.create(performance=perf, reservation=reservation, row=1, seat=5)
